...
def batch_predict(model):
	def batch_predict_local(images):
		model.eval()
		batch = torch.stack(tuple(preprocess_transform(i) for i in images), dim=0)
		logits = model(batch)
		probs = torch.nn.functional.softmax(logits, dim=1)
		return probs.detach().cpu().numpy()
	return batch_predict_local

im_path = args.image
im_name = im_path.split('/')[1][:-4]
im_path = os.path.join(PATH_DATA,im_path)
...
if args.interpreter == 'GradCAM':
	...
	explainer.explain(image, method_name='gradcam', target_layer_name=None, viz=True, save_path=None)
	explanation = explainer.result.numpy()[0,0]

elif args.interpreter == 'GradCAMpp':
	from xdeep.xlocal.gradient.explainers import *
	image = load_image(im_path)
	explainer = ImageInterpreter(model)
	explainer.explain(image, method_name='gradcampp', target_layer_name=None, viz=True, save_path=None) 
	explanation = explainer.result.numpy()[0,0]
	...
	image = load_image(im_path)
	explainer = ImageInterpreter(model)
	explainer.explain(image, method_name='scorecam', target_layer_name=None, viz=True, save_path=None) 
	explanation = explainer.result.numpy()[0,0]

elif args.interpreter == 'LIME':
	from lime import lime_image
	from utils.apply_lime import *
	img = get_image(im_path)
	explainer = lime_image.LimeImageExplainer()
    #HERE PREP IMAGE
	explanation = explainer.explain_instance(np.array(pill_transf(img)), 
                                         batch_predict(model), # classification function
                                         top_labels=4, 
                                         hide_color=0, 
	...
	l1_coeff = 0.01
	tv_coeff = 0.2
	original_img = cv2.imread(im_path, 1)
